...
Flow: `wrangler dev` → `wrangler types` (generates `Env`, never hand-write) → `wrangler deploy`. Secrets: `wrangler secret put NAME` (never in config/source). Pages production: always `--branch main`. Startup check: `wrangler check startup`.
